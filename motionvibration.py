...
def count_entries_by_zone(merged_df, start_time_filter=None, zone_filter=None):
    ...
    if zone_filter and zone_filter != 'All':
        merged_df = merged_df[merged_df['Zone'] == zone_filter]
...
if report_motion_file and report_vibration_file:
    ...
    with st.sidebar:
        ...
        # Zone filter with suggestive dropdown
        zones = ['All'] + list(merged_df['Zone'].unique())
        selected_zone = st.selectbox("Filter by Zone", options=zones)
    ...
    summary_df = count_entries_by_zone(merged_df, start_time_filter, selected_zone)
    ...
    # Show tables for prioritized zones (or all if "All" is selected)
    zones_to_display = zone_priority if selected_zone == 'All' else [selected_zone]

    for zone in zones_to_display:
        st.write(f"### {zone}")
