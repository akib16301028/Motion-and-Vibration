...
# Option to send notifications for other zones using a suggestive dropdown list
st.write("### Notifications for Other Zones")
selected_zone = st.selectbox(
    "Select Zone for Notification",
    options=merged_df['Zone'].unique(),
    help="Select a zone from the list for notifications"
)

if st.button("Send to Selected Zone"):
    # Process the selected zone
    concern = username_df[username_df['Zone'] == selected_zone]['Name'].values
    zonal_concern = concern[0] if len(concern) > 0 else "Unknown Concern"
    zone_df = merged_df[(merged_df['Zone'] == selected_zone) & (merged_df['Start Time'] >= start_time_filter)]
    
    if not zone_df.empty:
        message = f"<b>{selected_zone}:</b>\nAlarm came after: {start_time_filter.strftime('%Y-%m-%d %I:%M %p')}\n\n"
        ...
        if success:
            st.sidebar.success(f"Data for {selected_zone} sent to Telegram successfully!")
        else:
            st.sidebar.error(f"Failed to send data for {selected_zone} to Telegram.")
